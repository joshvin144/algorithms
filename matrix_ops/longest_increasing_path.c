...
static uint8_t search_step( uint8_t mat[TEST_ROWS][TEST_COLS], uint8_t mat_rows, uint8_t mat_cols, uint8_t row_idx, uint8_t col_idx )
{
	assert( NULL != mat );
	assert( mat_rows >= 1u );
	assert( mat_cols >= 1u );
	...
	if( search_up )
	{
		path_length_up = search_step( mat, mat_rows, mat_cols, row_idx - 1u, col_idx );
	}
	if( search_down )
	{
		path_length_down = search_step( mat, mat_rows, mat_cols, row_idx + 1u, col_idx );
	}
	if( search_right )
	{
		path_length_right = search_step( mat, mat_rows, mat_cols, row_idx, col_idx + 1u );
	}
	if( search_left )
	{
		path_length_left = search_step( mat, mat_rows, mat_cols, row_idx, col_idx - 1u );
	}
...
static uint8_t find_longest_increasing_path( uint8_t mat[TEST_ROWS][TEST_COLS], uint8_t mat_rows, uint8_t mat_cols )
{
	assert( NULL != mat );
	assert( mat_rows >= 1u );
	assert( mat_cols >= 1u );
	...
	for( uint8_t row_idx = 0u; row_idx <= ( mat_rows - 1u ); row_idx++ )
	{
		...
		{
			longest_increasing_path_returned = search_step( mat, mat_rows, mat_cols, row_idx, col_idx );
...
static void test_find_longest_increasing_path( void )
{
	...
	uint8_t longest_increasing_path_found = find_longest_increasing_path( test_mat, TEST_ROWS, TEST_COLS );
